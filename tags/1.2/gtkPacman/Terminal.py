...
class terminal(vte.Terminal):
    ...
    def do(self, areroot, action, args=None, name=None, msg=None):
        ...
        self.show()
        self.parent.show()
        self.gld.get_widget("expander").show_all()
        self.gld.get_widget("term_close_butt").hide()
        self.gld.get_widget("prog_bar").show()
        self.fork_command()
        self.feed_child(command+";exit \n")
        self.gui.set_stat_msg(msg)
        self.gui.start_pulse()
        
        return

    def exited(self, widget):
        """When a command exited, refresh the database to respect changes"""
    ...
    def close(self, event):
        ...
        self.gld.get_widget("expander").hide_all()
        self.gld.get_widget("prog_bar").hide()
        self.gld.get_widget("prog_bar").set_fraction(0.0)
